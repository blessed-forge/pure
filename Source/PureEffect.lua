...
function PureEffect:Create( windowName, parentWindow, effectTargetType )
    ...
    if( frame ~= nil ) then
    	...
    	frame.m_Windows					= 
      {
        ...
        [LINE]					= DynamicImage:CreateFrameForExistingWindow( baseName  .. "Line" ),
...
function PureEffect:Destroy()
	...
	do
		--d(frame.m_Name)
		--if (DoesWindowExist(frame.m_Name)) then
		--	d("exist")
		--end 
		frame:Destroy()
	end
...
function PureEffect:Initialize( windowDx, windowDy, frameAlpha, fontAlpha, fadeStartTime, fadeMinAlpha, showEffectDurations, showCooldownSpinner, showEffectMBorder, showLineEff, handleInput )
...
function PureEffect:SetEffect( effect )
	...
	if( self.m_effectData ~= nil ) then
        ...
        if( self.m_bShowLineEff ) then
        	local _, _, _, red, green, blue = DataUtils.GetAbilityTypeTextureAndColor( effect )
        	
        	if( red and green and blue ) then
        		self.m_Windows[LINE]:SetTintColor( red, green, blue )
        	else
        		self.m_Windows[LINE]:SetTintColor( 255, 255, 255 )
        	end
        end
